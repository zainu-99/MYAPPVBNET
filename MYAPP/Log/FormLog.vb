Public Class FormLog
    ...
    Sub ShowNavPageData()
        ...
        Dim query As String = "select top (select convert(int, ceiliNG(count(*)/ (convert(float," & ComboBoxEntries.Text & ")))) from LogApp a left join Users b on a.UserId =b.ID ) a.id from LogApp a left join Users b on a.UserId =b.ID  where (convert(date,[Datetime])>='" & DateTimePickerStart.Value.ToString("yyyy-MM-dd") & "' and convert(date,[Datetime]) <= '" & DateTimePickerUntil.Value.ToString("yyyy-MM-dd") & "') and (a.Action like '%" & TextBoxSearch.Text & "%' or a.TableName like '%" & TextBoxSearch.Text & "%' or a.DataBefore like '%" & TextBoxSearch.Text & "%' or a.DataAfter like '%" & TextBoxSearch.Text & "%' or a.Note like '%" & TextBoxSearch.Text & "%' or b.name like '%" & TextBoxSearch.Text & "%' )"
    ...
    Public Sub showDataGridView()
        Dim query As String = "select  a.Id,a.Action,a.TableName,a.DataBefore,a.DataAfter,a.Datetime,a.Note,concat(b.userid,'-',b.name) as [User] from LogApp a left join Users b on a.UserId =b.ID where (convert(date,[Datetime])>='" & DateTimePickerStart.Value.ToString("yyyy-MM-dd") & "' and convert(date,[Datetime]) <= '" & DateTimePickerUntil.Value.ToString("yyyy-MM-dd") & "') and (a.Action like '%" & TextBoxSearch.Text & "%' or a.TableName like '%" & TextBoxSearch.Text & "%' or a.DataBefore like '%" & TextBoxSearch.Text & "%' or a.DataAfter like '%" & TextBoxSearch.Text & "%' or a.Note like '%" & TextBoxSearch.Text & "%' or b.name like '%" & TextBoxSearch.Text & "%' ) order by a.Id desc OFFSET " & BindingSourcePaging.Position & " ROWS FETCH NEXT " & ComboBoxEntries.Text & " ROWS ONLY;"
